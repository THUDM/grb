...
import { getAttackChart, getDefenceChart, getTableColumns, getTableItems, getTableSelection } from './leaderboard'
...
export const AppLeaderboard = () => {
    ...
    const [leaderboard, setLeaderboard] = useState({ updated_time: null, data: {}, attacks: [], models: [], difficulties: [], attacksSummary: [], modelsSummary: [] })
    const [configs, setConfigs] = useState({ difficulties: [], attacks: [], models: [], attacksSummary: [], modelsSummary: [] })
    ...
    return <div className="app-leaderboard app-container" style={{ width: '100%', paddingTop: 30, paddingBottom: 30 }}>
        ...
        <div style={{ marginTop: 10, marginBottom: 10, display: "flex", justifyContent: "center" }}>
            ...
            <Tooltip title="Only raw models without any defence (layer normalization and adversarial training)">
                <Button style={{ width: 150, margin: 10 }} onClick={() => setConfigs({ ...configs, models: _.clone(leaderboard.models.filter(x => x.split('_').length === 1)) })}>No Defence</Button>
            </Tooltip>
        </div>
        <Divider style={{ marginTop: 50, fontSize: 24 }}>Comparison of Attacks and Defences</Divider>
        <div className="charts" style={{ width: '100%' }}>
            {[
                getAttackChart(leaderboard.data, leaderboard.difficulties, 'weighted', configs),
                getDefenceChart(leaderboard.data, leaderboard.difficulties, 'weighted', configs)
            ].map((chart, idx) =>
                <div key={idx} className="chart" style={{ width: 'calc(50% - 20px)', margin: 10, display: 'inline-block', height: `${80 * Math.max(configs.models.length, configs.attacks.length)}px` }}>
